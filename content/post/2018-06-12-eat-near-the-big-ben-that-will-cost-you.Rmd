...
What's the cost of eating near an iconic location?  We all suspect eating in a fun location is pricier.  I used the menu and restaurant location data that #MakeoverMonday "assigned" one week last December to demonstrate this among London [Wetherspoon Pubs](https://www.jdwetherspoon.com/).   #MakeoverMonday is a fun data visualization initiative; most participants use Tableau as their preferred visualization tool.  But I've used R and ggplot() and the organizers and participants have been very welcoming.  (Recently, I've noticed an initiative for R has sprouted up --- #TidyTuesday, with a focus on visualization and data wrangling. Might #MakeoverMonday have been an inspiration?  Probs.)

```{r echo=FALSE}
blogdown::shortcode('tweet', '939065290016555008')
```

The plot that submitted was popular with the organizers of #MakeoverMonday, and it seems that I will be featured in their soon to be published book on the #MakeoverMonday experience. 

```{r echo=FALSE}
blogdown::shortcode('tweet', '939073785994661888')
```

And I was happy to be working with some spatial data.  

```{r echo=FALSE}
blogdown::shortcode('tweet', '939405276314259456')
```

Basically, I did some mapping to explore the data, but suspected that having a closer look at London Pubs might also be interesting.  
...
g + geom_point(data = data0, 
              aes(x=Longitude, y=Latitude, group=NULL, fill=`Empire State Burger`), 
              colour="white", shape=21, stroke=1, alpha=.75, size = 2, shape = 25) + 
...
I decided to calculate restaurant distances from the Big Ben using the distm() function from the geosphere package ... 
```{r}
...
Now your're probably hungry and thirsty.  Find youself a burger or chicken tikka?
