...
This project breaks new ground in the ggplot2 ecosystem so that 'aggregation' can be as easily handled across *features* (columns) as it is to aggregegate across samples (rows).

```{r, echo = F, message = F, warning=F}
library(tidyverse)
penguins_normalized <- penguins |> 
  remove_missing() |> 
  mutate(bill_len = bill_len/ max(bill_len)) |>
  mutate(bill_dep = bill_dep/ max(bill_dep)) |> 
  mutate(flipper_len = flipper_len/max(flipper_len)) |> 
  mutate(body_mass = body_mass/max(body_mass)) 
```
...
The ggregions changes how analysts can interact with sf data.  Specifically it provides utilities that allow package authors to quickly and cleanly create region-aware `geom_*()` functions. 
...
User experience: Use geo-referenced in an intuitive way with smart layer --- borders are added under the hood! 
...
```{r, child = "../mytidytuesday/2026-01-21-gganatogram-x-ggregions/gganatogram-x-ggregions.Rmd"}
```
